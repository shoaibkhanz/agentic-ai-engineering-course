...
This pattern is different from others. A **Supervisor Agent** would have one agent explicitly delegate sub-tasks to worker agents [[9]](https://www.fiddler.ai/articles/multi-agent-llm-systems-for-enterprises). A **Sequential Pipeline** would force agents to always execute in a fixed order. In a **Peer-to-Peer** or **Network Architecture**, agents communicate dynamically without a central controller, offering flexibility but increasing coordination complexity [[9]](https://www.fiddler.ai/articles/multi-agent-llm-systems-for-enterprises). Our central LLM orchestration is simpler: the LLM acts as an intelligent tool selector rather than an explicit coordinator [[10]](https://openai.github.io/openai-agents-python/multi_agent/).
...
Let's break down the key steps. We first create a `FastMCP` instance for our composed server. Then, for each server defined in our configuration, we create a `Client` that connects to it, use `FastMCP.as_proxy(client)` to create a proxy object, and finally use `mcp.mount(proxy, prefix=prefix)` to mount it. The `mount()` method is what enables composition; it takes all capabilities from the proxy and adds them to the composed server with a specified prefix [[3]](https://gofastmcp.com/servers/composition), [[4]](https://gofastmcp.com/python-sdk/fastmcp-server-server).

### How Mounting Works

Mounting creates a live, dynamic link between the parent server and the subserver. Instead of copying components, requests for prefixed tools are delegated to the mounted server at runtime. This means any changes to the subserver are immediately reflected in the parent. For our local `stdio` servers, the performance overhead of this delegation is negligible. However, for remote servers connected over HTTP, this can introduce latency [[3]](https://gofastmcp.com/servers/composition).
...
In this lesson, you have integrated the Nova and Brown agents into a unified multi-agent system using MCP. We explored the Central LLM Orchestration pattern and implemented it in two ways: with a multi-server client and a composed server. You now understand the trade-offs of each approach and when to use them.

The power of MCP is its ability to make agent integration straightforward. Because both agents already expose their capabilities as MCP tools, we did not need to write any custom APIs or adapters. This modularity is a core value of MCP: you can write agents once and integrate them anywhere.

In the next lesson, you will see a video demonstration of using both agents from Cursor to produce an article end-to-end. In Part 3 of this course, we will cover production engineering practices, including how to deploy these composed servers remotely, add monitoring and observability, and implement security measures.

## References

1. Berkeley Function Calling Leaderboard. (n.d.). Gorilla. https://gorilla.cs.berkeley.edu/leaderboard.html

2. Evaluating Tool Calling Capabilities in Large Language Models: A Literature Review. (n.d.). Quotient AI. https://blog.quotientai.co/evaluating-tool-calling-capabilities-in-large-language-models-a-literature-review/

3. Server Composition. (n.d.). FastMCP. https://gofastmcp.com/servers/composition

4. The FastMCP Server. (n.d.). FastMCP. https://gofastmcp.com/python-sdk/fastmcp-server-server

5. Multi-Agent Orchestration with Reinforcement Learning. (n.d.). arXiv. https://arxiv.org/abs/2505.19591

6. Designing Multi-Agent Intelligence. (n.d.). Microsoft Developer Blog. https://developer.microsoft.com/blog/designing-multi-agent-intelligence

7. LLM Multi-Agent Reinforcement Learning. (n.d.). Xue Guang. https://xue-guang.com/post/llm-marl/

8. Multi-Agent Research System. (n.d.). Anthropic. https://www.anthropic.com/engineering/multi-agent-research-system

9. Multi-Agent LLM Systems for Enterprises. (n.d.). Fiddler AI. https://www.fiddler.ai/articles/multi-agent-llm-systems-for-enterprises

10. Multi-Agent. (n.d.). OpenAI Agents Python. https://openai.github.io/openai-agents-python/multi_agent/

11. Architecture. (n.d.). Model Context Protocol. https://modelcontextprotocol.io/docs/learn/architecture

12. Architectures for Multi-Agent Systems. (n.d.). Galileo AI. https://galileo.ai/blog/architectures-for-multi-agent-systems

13. A Practical Guide to Building Agents. (n.d.). OpenAI. https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf

14. LLM Agent Orchestration with LangChain and Granite. (n.d.). IBM Think. https://www.ibm.com/think/tutorials/llm-agent-orchestration-with-langchain-and-granite

15. AI Agent Design Patterns. (n.d.). Microsoft Learn. https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/ai-agent-design-patterns

16. Model Context Protocol Specification. (2025, November 25). Model Context Protocol. https://modelcontextprotocol.io/specification/2025-11-25