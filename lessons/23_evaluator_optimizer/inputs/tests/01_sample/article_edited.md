...
An LLM workflow is a sequence of tasks involving LLM calls or other operations, such as reading or writing data to a database or file system [[2]](https://substack.com/home/post/144675713). Developer-written code largely orchestrates these tasks. The steps are defined in advance, resulting in deterministic or rule-based paths with predictable execution and explicit control flow. Think of a workflow as a factory assembly line, where each step is precisely engineered and follows a predefined order.

In future lessons, we will explore concepts such as chaining and routing, where multiple LLM calls are automated together. We will also look at orchestrator-worker patterns, which can dynamically plan and call multiple actions. Workflows are adaptable in their complexity, ranging from simple prompt chains to sophisticated multi-step processes, providing a solid foundation for many production AI systems [[1]](https://www.anthropic.com/engineering/building-effective-agents), [[7]](https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/).
...
Image 1: A simple Retrieval-Augmented Generation (RAG) workflow. (Source [Iusztin, P. (n.d.). Exploring the difference between agents and workflows. Decoding AI Magazine. [[2]](https://decodingml.substack.com/p/llmops-for-production-agentic-rag)])

### AI Agents

AI agents are systems where an LLM (or multiple LLMs) dynamically decides the sequence of steps, reasoning, and actions to achieve a goal [[1]](https://www.anthropic.com/engineering/building-effective-agents). The steps are not defined in advance. Instead, they are dynamically planned based on the task and the current state of the environment. An agent is adaptive, capable of handling novelty, and driven by LLM autonomy in decision-making and execution path. Consider an agent as a skilled human expert tackling an unfamiliar problem, adapting in real time after each "Eureka!" moment.

Later in this course, we will explore agentic concepts like tools, memory, and ReAct agents. Agents represent a higher level of autonomy, allowing LLMs to interact with their environment and perform multi-step tasks without explicit, predefined instructions [[3]](https://substack.com/home/post/144675713).
...
Image 2: The components of an LLM-powered agent. (Image by author from [Towards Data Science. (n.d.). A Developer’s Guide to Building Scalable AI: Workflows vs Agents. [[7]](https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/)])

### The Role of Orchestration

Both workflows and agents require an orchestration layer, but their nature differs significantly. In workflows, the orchestration layer executes a predefined plan. For agents, it facilitates the LLM's dynamic planning and execution, acting as a guide rather than a strict controller. This distinction determines the level of control developers maintain over the system's behavior.
...
Image 3: Differences between workflows and agents. (Source [Iusztin, P. (n.d.). Exploring the difference between agents and workflows. Decoding AI Magazine. [[2]](https://decodingml.substack.com/p/llmops-for-production-agentic-rag)])
...
AI agents excel in open-ended research and synthesis, like researching World War II [[7]](https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/). They are also suitable for dynamic problem-solving, such as debugging code or handling complex customer support interactions. Interactive task completion in unfamiliar environments, like booking a flight without specifying the exact websites to use, is another area where agents shine.
...
Tools like Cursor use this autonomy slider. For instance, in Cursor, `CMD+K` allows you to change a selected chunk of code, `CMD+L` changes the entire file, and `CMD+I` lets the AI autonomously change the entire repository [[5]](https://www.youtube.com/watch?v=LqjP7w4g4iY). Similarly, Perplexity offers varying levels of autonomy with "search," "research," and "deep research" options.

The ultimate goal is to speed up the AI generation and human verification loop. This is often achieved through good workflow or agentic architecture and well-designed user interface (UI) and user experience (UX) platforms, like Cursor for coding [[5]](https://www.youtube.com/watch?v=LqjP7w4g4iY).
...
*   **Chaining and routing** automate multiple LLM calls together. As a first automation step, it helps glue together multiple LLM calls and decide between multiple appropriate options [[1]](https://www.anthropic.com/engineering/building-effective-agents).
...
*   **Orchestrator-worker** understands user intent, dynamically plans, calls multiple actions, and synthesizes them into a final answer [[1](https://www.anthropic.com/engineering/building-effective-agents)]. It allows the AI program to dynamically decide what actions to take based on the given task, making a smooth transition between the workflow and agentic world.
...
To better anchor you in the world of LLM workflows and AI agents, we introduce some concrete examples, from a simple workflow to a single agent system and a more advanced hybrid solution. We explain everything as if speaking to a 7-year-old, without any fancy acronyms, since you are only familiar with what was explained previously in this lesson.
...
```
Image 10: The operational loop of the Gemini CLI coding assistant. (Source [Google Gemini. (n.d.). Gemini CLI README.md. GitHub. [[4]](https://github.com/google-gemini/gemini-cli/blob/main/README.md)])
...
```
Image 11: The iterative multi-step process of Perplexity Deep Research. (Source [Karpathy, A. (n.d.). Andrej Karpathy: Software in the Era of AI. YouTube. [[5]](https://www.youtube.com/watch?v=LqjP7w4g4iY)])
...
*   **Context Limits**: Systems struggle to maintain coherence across long conversations, gradually losing track of their purpose. Ensuring consistent output quality across different agent specializations remains a continuous challenge.
...
*   **Cost-Performance Trap**: Sophisticated agents deliver impressive results but cost a fortune per user interaction, making them economically impractical for many applications.
*   **Security Concerns**: Autonomous agents with powerful write permissions could send incorrect emails, delete important files, or expose sensitive data.
...
6. Google Developers. (n.d.). Google announces Gemini CLI, your open-source AI agent. Google Developers. [https://googledevelopers.blog/google-announces-gemini-cli-your-open-source-ai-agent/](https://googledevelopers.blog/google-announces-gemini-cli-your-open-source-ai-agent/)
7. Towards Data Science. (n.d.). A Developer’s Guide to Building Scalable AI: Workflows vs Agents. Towards Data Science. [https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/](https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/)
</article>